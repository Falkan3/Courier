...
function popup (Courier, Components, Events) {
  ...
  var Popup = {
    ...
    onAppRendered: function onAppRendered(event) {// Only run for elements with the #courierPopup ID
  ...
  Events.on(['destroy', 'popup.mount.before'], function () {
    objectForEach(Popup.refs, function (item) {
      if (item.el.parentNode) {
        item.el.parentNode.removeChild(item.el);
      }
    });
...
export { chat as Chat, popup as Popup };
